$('document').ready(function() {
    ...
    $.post('lib/util/getQEoptions.php', function(data) {

        $.each(data, function(label, array) {
           switch (label) {
               case 'tender':
                   div = "#tender, #transferTender";
                   break;
               case 'type':
                   div = '#qeType';
                   break;
    ...
    function figureDailyBudget() {
        ...
        $.post('lib/util/figureDailyBudget.php',function(data) {
    ...
    function getBalanceScreen() {
        ...
        $.post('lib/php/balanceScreen.php', function(data) {
            ...
            $.post('lib/util/getReconcile.php', function(data) {
                $('#reconHome').html(data.summary);

                $.each(data.found, function(i, v) {

                    //console.log('found trans: ' + v);

                    i == 0 ? $('#reconFound').html(v) : $('#reconFound').append(v);
                });

                $.each(data.notFound, function(i, v) {

                    //console.log('notFound trans: ' + v);
                    i == 0 ? $('#reconNotFound').html(v) : $('#reconNotFound').append(v);
                });
    ...
    function reconcileTrans(sendData) {
        console.log(sendData);

        $.post('lib/util/processRecon.php', sendData, function(data) {
            if(data.error == true) {
                alert('error updating information');
            } else {
                removeDiv = data.divID;
    ...
    function parsePDF(fileName) {
        ...
        parser = 'lib/pdfParsers/pdfConvert.php';

        $.post(parser, {'fileName' : fileName, 'action': 'process'}, function(data) {

            $.post(parser, {'jobID': data.id, 'fileName': data.fileName, 'action': 'getJob'}, function(data) {

                if(data.fileID != 'error') {
                    $.post(parser, {'fileID': data.fileID, 'fileName': data.fileName, 'action': 'getFile'}, function(data) {
    ...
    $('body').on('click', '.recurPayBtn', function() {
        ...
        formData = $('#recurPaymentForm-' + transID).serialize();

        console.log(formData);

        $.post('lib/util/updateUpcomingX.php', {'transID': transID, 'formData': formData}, function() {
            console.log('upcomingX.php updated');
            $('#' + transID).parent().remove();
        });

        if(status == 'payNow') {
            $.post('lib/util/processQuickEntry.php', formData, function(data) {
    ...
    //process recon buttons
    $('body').on('click', '.reconBtn', function() {

        parentDiv = $(this).parent().parent().attr('id');
        found = $('#' + parentDiv).data('found');
        button    = $(this).data('button');
        trans     = $(this).data('trans');
        action    = $(this).data('action');

        console.log("trans: " + trans + "; button: " + button);

        $('#' + parentDiv + ' > .matchedData').each(function() {

            trans  = $(this).data('trans');
            id     = $(this).data('id');
            button = $(this).data('button');

            console.log('matchedData trans: ' + trans);
            console.log('matchedData id: ' + id);
            console.log('matchedData button: ' + button);

            button == 'recon' ? reconID = id : entryID = id;

        });

        found ? sendData = {'reconID': reconID, 'entryID': entryID, 'divID': parentDiv, 'action': action, 'found': found} :
        sendData = {'reconID': reconID, 'divID': parentDiv, 'action': action, 'found': found};

        switch(action) {
            case 'recon':
                recon = confirm(
                    'This Action Will Mark The Trans As Reconciled.  Press Confirm To Keep Trans Record But Remove From This Screen');
                recon ? reconcileTrans(sendData) : console.log("don't recon trans");
                break;
            case 'dismiss':
                dismiss = confirm(
                    'This Action Will Remove From Current Screen But Will Make Visible The Next Time This Page Loads.  To Permanently Remove Transaction Cancel This And Press Delete.');
                dismiss ? $('#' + parentDiv).remove() : console.log('do not dismiss trans');
                break;
            case 'delete':
                remove = confirm(
                    'This Action Will Permanently Remove Trans & Will Not Be Able To Be Retrieved.  To Termporarily Remove Transaction Press Cancel And Press Dismiss');
                remove ? reconcileTrans(sendData) : console.log('no not remove trans');
                break;
            case 'add':
                add = confirm("Press Confirm To Enter This As A New Transaction");
                add  ? console.log('add transaction') : console.log("don't add trans");
                break;
            default:
        ...
        //button == 'recon' ? reconcileTrans(sendData) : $('#' + parentDiv).remove();

        console.log(sendData);

    });

    //process filename submission & parse pdf
    $('body').submit('#processFileName', function(event) {
        event.preventDefault();
